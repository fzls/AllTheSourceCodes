...
import javax.swing.*;
           ...
           public class specialstudent1 extends JFrame implements ActionListener{
             ...
             //  Object col[]={"课程代码","课程名称","学分","学期","学年"};
           	Object col[]={"ID","NAME","GENDER","BIRTHDAY","PROVINCE","CITY"};
               JTable table;      //建表
               specialstudent1(String s){
           	super(s);
           	 ...
           	 int i=0;
           	  for(int j=0;j<20;j++)//将新的结果写上去
            	  ...
            	  try{
                       ...
                       ResultSet rs=sql.executeQuery( "SELECT * FROM student WHERE s_id in (select s_id from sc,course 	where sc.c_id = course.c_id and score > 60 group by s_id having sum(credit) = (SELECT top 1 sum(credit) FROM sc, course WHERE sc.c_id=course.c_id And score>60 GROUP BY s_id ORDER BY sum(credit) DESC))");      
            		  while(rs.next())
            		   {
            		  ...
            		  String r=rs.getString(6);
            		 	...
            		 	a[i][3]=xueqi;a[i][4]=xuenian;a[i][5]=r;
            		 	i++;
            		   }pack();
            		   con.close();
