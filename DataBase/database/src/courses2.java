...
Object col[]={"ID","NAME","GENDER","DEPARTMENT","PROVINCE/CITY","REGION"};
JTable table;      //建表
//Connection ccon;
//Statement sql; ResultSet rs;

    public courses2(String s) {
    	super(s);
    ...
    if(e1.getSource()==button1){
     int i=0;
      ...
      try{
          ...
          ResultSet rs=sql.executeQuery("SELECT * FROM student WHERE s_id in (select s_id from sc where score < 60 And ayear LIKE '%"+s1+"%' group by s_id having count(*) >= 2) And  dept_id LIKE '%"+s2+"%'");
          while(rs.next())
           {
         ...
         //   String fenshuduanrenshu=rs.getString(6);
           // Date ruxiaoshijian=rs.getDate(6);
            ...
            a[i][3]=xibie;a[i][4]=xuenian;a[i][5]=GPA;
            i++;
           }pack();
           con.close();
